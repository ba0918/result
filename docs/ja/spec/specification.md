...
`Result` / `Option` / `Exception` の選択は、1つの問いに従います。
「呼び出し側は、この失敗を通常の分岐として扱いたいだろうか？」

- 呼び出し側が分岐すべき失敗（バリデーション・業務ルール）→ `Result` / `Option`
- 理由が不要な正常な「値なし」→ `Option`
- 不変条件違反・プログラミングミス → `Exception`
- この層では回復不能なインフラ障害（ファイル・DB・ネットワーク）→ 原則 `Exception`
- 上位層でリトライや代替処理を行う失敗 → 境界で `Result` に変換

詳細な判断表とチェーン設計の指針（1チェーン=1責務、andThenは4段程度まで、
andThenの入れ子禁止）は[ベストプラクティス集](../guide/best_practices.md)を参照。
...
- バージョンの正準位置はgitタグ（例: `v1.1.0`）。`composer.json`には`version`フィールドを持たず、Composerはタグからバージョンを導出する
- `CHANGELOG.md`の見出しはタグと完全に一致させる。各リリースは「バージョン見出し・比較リンク・タグ」をワンセットで発行する
- 既存の挙動の意味を変える変更はchangelogで**Breaking**と明記し、メジャーバージョンを上げる
...
- ✅ `is_ok_and()` / `is_err_and()` - 成功/失敗判定+条件チェック
- ✅ `map()` / `map_err()` - 値/エラーの変換
- ✅ `map_or()` / `map_or_else()` - 成功時変換+失敗時デフォルト値
- ✅ `and_then()` - モナド的チェーン処理
...
### 概要
コードの簡潔性と可読性を向上させるためのショートハンドメソッド群です。これらのメソッドは既存メソッドの組み合わせを一つのメソッドで実現し、一般的なパターンを効率化します。
