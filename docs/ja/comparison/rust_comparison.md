...
> **注記**: この例はRust標準ライブラリの機械的な対応を示したものです。
> RustではファイルI/Oの失敗も `Result` で表現されます。PHPのプロジェクトで
> どの失敗を `Result` に載せるか（呼び出し側が分岐したい失敗だけにするか）は
> 別の設計判断であり、[ベストプラクティス集](../guide/best_practices.md) を参照してください。
...
> **注記**: ここでも同じ原則が当てはまります。ネットワーク障害はインフラ
> 障害です。この例はRustとの機械的な対応を示したもので、例外を `Err` に
> 変換するのは、呼び出し側がその失敗で意味のある分岐を持てる場合に限り
> 有用です。[ベストプラクティス集](../guide/best_practices.md) を参照してください。
